...
async function onSignedIn() {
  ...
  // loadLocal() already called in init() — don't call again here
  // to avoid resetting any in-memory pending ops
...
- competencyLevel (one of: "Observed", "Assisted", "Performed (Supervised)", "Performed (Independent)")
...
function renderForm(data) {
  ...
  const compChips  = compLevels.map(l => {
    ...
    const key = l === 'Performed (Supervised)'  ? 'supervised'  :
                l === 'Performed (Independent)' ? 'independent' : l.toLowerCase();
...
function selectComp(el, val) {
  ...
  const key = val === 'Performed (Supervised)'  ? 'supervised'  :
              val === 'Performed (Independent)' ? 'independent' : val.toLowerCase();
...
// ============================================================
// INIT — single onAuthStateChange listener, no race conditions
// ============================================================
(async function init() {
  loadLocal();

  // One listener handles everything: initial page load, refresh, sign-in, sign-out.
  // INITIAL_SESSION fires on every page load (including refresh) with the stored session.
  // No competing getSession() / refreshSession() calls needed.
  let appStarted = false; // guard against double INITIAL_SESSION + SIGNED_IN

  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
      if (appStarted) return; // already handled
      appStarted = true;
      if (session?.user) {
      ...
      // Token silently refreshed — update user ref but don't re-run onSignedIn
