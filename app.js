...
function setSyncStatus(status) {
  ...
  const msgs = {
    ...
    offline: 'Offline — entries saved locally, will sync when online'
  };
  textEl.textContent = msgs[status] || '';
}
    ...
    try {
      if (op.type === 'upsert') {
        ...
        if (entry) {
          ...
          const { error } = await sb.from('procedure_entries').upsert(row, { onConflict: 'id' });
      ...
      console.warn('Sync failed for', op.id, e);
    }
  }
...
async function syncFromCloud() {
  ...
  try {
    ...
    // Merge: cloud entries win unless local has a pending upsert
    const pendingIds = new Set(pendingSync.filter(p => p.type === 'upsert').map(p => p.id));
    const cloudEntries = (data || []).map(rowToEntry);

    // Keep local-pending entries, replace everything else with cloud
    const localPending = entries.filter(e => pendingIds.has(e.id));
    const cloudById = {};
    cloudEntries.forEach(e => { cloudById[e.id] = e; });
    localPending.forEach(e => { cloudById[e.id] = e; }); // local overrides for pending
...
function setAuthLoading(btnId, loading, label) {
  ...
  btn.innerHTML = loading ? `<span class="spinner"></span>Signing in…` : label;
}
...
async function doReset() {
  ...
  if (error) {
    msgEl.textContent = error.message; msgEl.style.color = 'var(--danger)';
  } else {
...
async function onSignedIn() {
  // T&C check
  if (!localStorage.getItem(TC_KEY)) {
    openModal('tcModal');
  }
  ...
  const { data: profData } = await sb.from('user_profiles')
...
function classifyCD(text) {
  ...
  if (/iv antibiotic|blood transfus|total parenteral|DVT|PE|atrial fib|pneumonia/.test(t)) return 'II';
...
const CD_DESC   = {
  I:   'Minor — managed without medication beyond analgesics/antipyretics',
  II:  'Pharmacological treatment required (IV antibiotics, transfusion, etc.)',
  III: 'Surgical, endoscopic or radiological re-intervention required',
  IV:  'Life-threatening — requires ICU management',
  V:   'Death of patient'
};
...
function parseNLP(text) {
  const result = {};
  const t = text.toLowerCase();
  const match = PROC_CODES.find(p => t.includes(p.name.toLowerCase()));
  if (match) {
    result.procedureName = match.name;
    result.procedureCode = `OPCS: ${match.opcs} / CPT: ${match.cpt}`;
    result.specialty = match.specialty;
    result.procedureType = match.type;
  }
  const surm = text.match(/(?:with |Mr |Ms |Mrs\.|Dr\.|Prof\.?)([A-Z][a-z]+(?: [A-Z][a-z]+)?)/);
  if (surm) result.surgeonName = surm[1];
  if (/\bobserved?\b/.test(t)) result.competencyLevel = 'Observed';
  else if (/\bassisted?\b/.test(t)) result.competencyLevel = 'Assisted';
  else if (/\bsupervised?\b/.test(t)) result.competencyLevel = 'Performed (Supervised)';
  else if (/\bindependent\b/.test(t)) result.competencyLevel = 'Performed (Independent)';
  const durm = text.match(/(\d+)\s*(?:min|minutes?|hrs?|hours?)/i);
  if (durm) { let n = parseInt(durm[1]); if (/hr|hour/i.test(durm[0])) n *= 60; result.duration = String(n); }
  if (/no complica|uncomplicated|nil/.test(t)) result.complications = 'None';
  else { const cm = text.match(/complication[s]?[:\s]+([^.]+)/i); if (cm) result.complications = cm[1].trim(); }
  if (/today/.test(t)) result.date = todayISO();
  else if (/yesterday/.test(t)) { const d = new Date(); d.setDate(d.getDate()-1); result.date = d.toISOString().slice(0,10); }
  return result;
}
...
function setFilter(val) { currentFilter = val; renderFilters(); renderLog(); }

function renderLog() {
  const q = (document.getElementById('searchInput')?.value || '').toLowerCase().trim();
  let filtered = entries.filter(e => {
    const matchFilter = currentFilter === 'all' || e.specialty === currentFilter;
    const matchSearch = !q || [e.procedureName, e.specialty, e.surgeonName, e.procedureType, e.notes, e.complications]
  ...
  const isPending = id => pendingSync.some(p => p.id === id);
  ...
  document.getElementById('entryList').innerHTML = filtered.map(e => `
    <div class="entry-wrap" id="wrap-${e.id}">
      ...
      <div class="card" onclick="openDetail('${e.id}')"
        ...
        <div class="card-row">
          <div class="entry-name">${e.procedureName || 'Untitled'}</div>
          <div class="entry-date">${formatDate(e.date)}</div>
        </div>
        <div class="entry-meta">
          ${e.specialty ? `<span class="tag tag-specialty">${e.specialty}</span>` : ''}
          ${e.competencyLevel ? `<span class="tag ${compClassMap[e.competencyLevel]||''}">${compLabelMap[e.competencyLevel]||e.competencyLevel}</span>` : ''}
          ${e.signatureData ? `<span class="tag tag-signed">✓ Signed</span>` : ''}
          ${isPending(e.id) ? `<span class="tag tag-pending">⏳ Pending sync</span>` : ''}
        </div>
        ${e.surgeonName ? `<div style="font-size:12px;color:var(--muted);margin-top:6px;">With ${e.surgeonName}</div>` : ''}
      </div>
    </div>`).join('');
}
...
function touchMove(e, id) {
  ...
  if (dx < -10) { const c = document.querySelector(`#wrap-${id} .card`); if (c) c.style.transform = `translateX(${Math.max(dx,-80)}px)`; }
}
...
function openDetail(id) {
  ...
  const cdHtml = cd ? `<div class="cd-badge cd-${cd}">${CD_LABELS[cd]} — ${CD_DESC[cd]}</div>` : '';

  document.getElementById('detailContent').innerHTML = `
    <div class="detail-title">${e.procedureName || 'Untitled Procedure'}</div>
    <div class="detail-date">${formatDate(e.date)}</div>
    <div class="card" style="margin-bottom:10px;">
      ${row('Date', formatDate(e.date))}
      ${row('Procedure', e.procedureName)}
      ${row('Code', e.procedureCode)}
      ${row('Type', e.procedureType)}
      ${row('Specialty', e.specialty)}
      ${row('Surgeon / Supervisor', e.surgeonName)}
      ${row('Role / Title', e.roleTitle)}
      ${row('Reg. Number', e.regNumber)}
      ${row('Competency', e.competencyLevel)}
      ${row('Duration', e.duration ? e.duration + ' min' : '')}
    </div>
    ${e.complications ? `<div class="card" style="margin-bottom:10px;"><div class="form-section-title">Complications</div><div style="font-size:14px;">${e.complications}</div>${cdHtml}</div>` : ''}
    ${e.outcome ? `<div class="card" style="margin-bottom:10px;"><div class="form-section-title">Outcome / Notes</div><div style="font-size:14px;">${e.outcome}</div></div>` : ''}
    ${e.followUpDate ? `<div class="card" style="margin-bottom:10px;">${row('Follow-up Due', formatDate(e.followUpDate))}${e.followUpNotes ? row('Follow-up Notes', e.followUpNotes) : ''}</div>` : ''}
    ${e.signatureData ? `<div class="card" style="margin-bottom:10px;"><div class="form-section-title">Supervisor Sign-off</div><img src="${e.signatureData}" class="sig-preview" alt="Signature">${e.supervisorName ? `<div style="font-size:12px;color:var(--muted);margin-top:8px;">${e.supervisorName}</div>` : ''}</div>` : ''}
    <div class="btn-row">
      <button class="btn btn-secondary" onclick="openEdit('${id}')">Edit</button>
      <button class="btn btn-danger" onclick="deleteEntry('${id}'); headerAction();">Delete</button>
    </div>
...
function openAdd() {
  editingId = null;
  const defaults = { roleTitle: userProfile.roleTitle || '', regNumber: userProfile.regNumber || '' };
  renderForm(defaults);
...
function renderForm(data) {
  const compLevels = ['Observed','Assisted','Performed (Supervised)','Performed (Independent)'];
  const compChips = compLevels.map(l => {
    const sel = data.competencyLevel === l;
    const key = l === 'Performed (Supervised)' ? 'supervised' : l === 'Performed (Independent)' ? 'independent' : l.toLowerCase();
    return `<div class="comp-chip ${sel ? 'sel-'+key : ''}" onclick="selectComp(this,'${l}')">${l}</div>`;
  }).join('');

  document.getElementById('formContent').innerHTML = `
    <div class="form-section">
      <div class="form-section-title">✨ Smart Entry</div>
      <div class="nlp-row">
        <input class="nlp-input" id="nlpInput" placeholder="e.g. lap chole with Mr Smith, assisted, 45 min…">
        <button class="nlp-btn" onclick="runNLP()">✨</button>
      </div>
    </div>
    <div class="form-section">
      <div class="form-section-title">Procedure Details</div>
      <div class="field"><label>Date<span class="required-star">*</span></label><input type="date" id="f_date" value="${data.date || todayISO()}"></div>
      <div class="field">
        <label>Procedure Name<span class="required-star">*</span></label>
        <input type="text" id="f_procedureName" value="${esc(data.procedureName||'')}" placeholder="e.g. Laparoscopic Cholecystectomy" autocomplete="off" oninput="showProcSuggestions(this.value)">
        <div class="autocomplete-list" id="procList"></div>
      </div>
      <div class="field"><label>Procedure Code (OPCS / CPT)</label><input type="text" id="f_procedureCode" value="${esc(data.procedureCode||'')}" placeholder="e.g. OPCS: J18.3 / CPT: 47562"></div>
      <div class="field"><label>Procedure Type</label><input type="text" id="f_procedureType" value="${esc(data.procedureType||'')}" placeholder="e.g. Laparoscopic, Open"></div>
    ...
    <div class="form-section">
      ...
      <div class="field"><label>Surgeon / Supervisor Name</label><input type="text" id="f_surgeonName" value="${esc(data.surgeonName||'')}" placeholder="e.g. Mr A Smith"></div>
      <div class="field"><label>Your Role / Title</label><input type="text" id="f_roleTitle" value="${esc(data.roleTitle||'')}" placeholder="e.g. SFA, Scrub Nurse, ODP"></div>
      <div class="field"><label>Registration Number (NMC / HCPC / FPA)</label><input type="text" id="f_regNumber" value="${esc(data.regNumber||'')}" placeholder="e.g. NMC 12A3456B"></div>
    ...
    <div class="form-section">
      ...
      <div class="field"><label>Duration (minutes)</label><input type="number" id="f_duration" value="${esc(data.duration||'')}" placeholder="e.g. 90" min="1" max="999"></div>
      <div class="field"><label>Complications</label><textarea id="f_complications" oninput="updateCDPreview(this.value)" placeholder="Describe complications, or 'None'…">${esc(data.complications||'')}</textarea><div id="cdPreview"></div></div>
      <div class="field"><label>Outcome / Notes</label><textarea id="f_outcome" placeholder="Post-op outcome, learning points…">${esc(data.outcome||'')}</textarea></div>
    </div>
    <div class="form-section">
      <div class="form-section-title">30-Day Follow-up</div>
      <div class="field"><label>Follow-up Due Date</label><input type="date" id="f_followUpDate" value="${data.followUpDate||''}"></div>
      <div class="field"><label>Follow-up Notes</label><textarea id="f_followUpNotes" placeholder="Planned follow-up, concerns…">${esc(data.followUpNotes||'')}</textarea></div>
    </div>
    <div class="form-section">
      <div class="form-section-title">Supervisor Sign-off</div>
      <div class="field"><label>Supervisor Name</label><input type="text" id="f_supervisorName" value="${esc(data.supervisorName||'')}" placeholder="Supervisor's name"></div>
      <div class="field">
  ...
  requestAnimationFrame(() => initSigCanvas(data.signatureData));
}

function initSigCanvas(existingData) {
  ...
  sigCtx.strokeStyle = '#E6EDF3'; sigCtx.lineWidth = 2; sigCtx.lineCap = 'round'; sigCtx.lineJoin = 'round';
  ...
  sigCanvas.addEventListener('touchmove', e => { e.preventDefault(); if (!sigDrawing) return; const p = getPos(e); sigCtx.lineTo(p.x, p.y); sigCtx.stroke(); sigHasData = true; document.getElementById('sigLabel').style.opacity='0'; }, { passive: false });
  sigCanvas.addEventListener('touchend', () => sigDrawing = false);
}

function clearSig() {
  if (sigCtx) sigCtx.clearRect(0, 0, sigCanvas.width/window.devicePixelRatio, sigCanvas.height/window.devicePixelRatio);
  sigHasData = false;
...
function selectComp(el, val) {
  ...
  const key = val === 'Performed (Supervised)' ? 'supervised' : val === 'Performed (Independent)' ? 'independent' : val.toLowerCase();
...
function updateCDPreview(val) {
  ...
  const el = document.getElementById('cdPreview');
  if (!el) return;
...
function showProcSuggestions(val) {
  ...
  const matches = PROC_CODES.filter(p => p.name.toLowerCase().includes(q)).slice(0,5);
...
function runNLP() {
  const txt = document.getElementById('nlpInput').value;
  if (!txt.trim()) { toast('Type a description first'); return; }
  const parsed = parseNLP(txt);
  if (parsed.procedureName) document.getElementById('f_procedureName').value = parsed.procedureName;
  if (parsed.procedureCode) document.getElementById('f_procedureCode').value = parsed.procedureCode;
  if (parsed.specialty) { const s = document.getElementById('f_specialty'); for (let o of s.options) if (o.value === parsed.specialty) { o.selected = true; break; } }
  ...
  if (parsed.duration) document.getElementById('f_duration').value = parsed.duration;
  if (parsed.complications) { document.getElementById('f_complications').value = parsed.complications; updateCDPreview(parsed.complications); }
  ...
  if (parsed.competencyLevel) {
    ...
    document.querySelectorAll('.comp-chip').forEach(c => {
      ...
      if (c.textContent.trim() === parsed.competencyLevel) {
        const key = parsed.competencyLevel === 'Performed (Supervised)' ? 'supervised' : parsed.competencyLevel === 'Performed (Independent)' ? 'independent' : parsed.competencyLevel.toLowerCase();
...
function renderDashboard() {
  ...
  const monthCounts = {};
  entries.forEach(e => { if (e.date) { const m = e.date.slice(0,7); monthCounts[m] = (monthCounts[m]||0)+1; } });
  ...
  const monthBars = months.map(m => {
    ...
    return `<div class="bar-row"><div class="bar-label">${label}</div><div class="bar-track"><div class="bar-fill" style="width:${(monthCounts[m]/maxM*100).toFixed(1)}%"></div></div><div class="bar-count">${monthCounts[m]}</div></div>`;
  }).join('');

  const specCounts = {};
  entries.forEach(e => { if (e.specialty) specCounts[e.specialty] = (specCounts[e.specialty]||0)+1; });
  ...
  const compCounts = { Observed: 0, Assisted: 0, 'Performed (Supervised)': 0, 'Performed (Independent)': 0 };
  entries.forEach(e => { if (e.competencyLevel && compCounts[e.competencyLevel] !== undefined) compCounts[e.competencyLevel]++; });
  const compColors = { Observed: 'var(--comp2)', Assisted: 'var(--comp3)', 'Performed (Supervised)': 'var(--comp4)', 'Performed (Independent)': 'var(--comp1)' };
  const compLegend = Object.entries(compCounts).filter(([,n])=>n>0).map(([k,n]) =>
  ...
  const total = entries.length;
  const compEntries = Object.entries(compCounts).filter(([,n])=>n>0);
  let donutSvg = '';
  if (compEntries.length) {
    const circ = 2 * Math.PI * 40;
    let offset = 0;
    const arcs = compEntries.map(([k,n]) => {
    ...
    donutSvg = `<svg viewBox="0 0 100 100" width="120" height="120" style="flex-shrink:0"><circle cx="50" cy="50" r="40" fill="none" stroke="var(--surface2)" stroke-width="14"/>${arcs.join('')}<text x="50" y="54" text-anchor="middle" fill="var(--text)" font-size="16" font-family="Playfair Display,serif" font-weight="700">${total}</text></svg>`;
  }

  document.getElementById('dashboardContent').innerHTML = `
    <div class="dash-section-title">Cases per Month</div>
    <div class="card">${monthBars || '<p style="color:var(--muted);font-size:13px;">Not enough data</p>'}</div>
...
function renderFollowUps() {
  ...
  const withFollowUp = entries.filter(e => e.followUpDate).map(e => {
    let badge;
    if (e.followUpDate < today) badge = `<span class="overdue-badge">Overdue</span>`;
    else if (e.followUpDate <= soonISO) badge = `<span class="due-soon-badge">Due soon</span>`;
    else badge = `<span class="done-badge">Upcoming</span>`;
    return { ...e, badge };
  }).sort((a,b) => a.followUpDate.localeCompare(b.followUpDate));

  document.getElementById('followupContent').innerHTML = withFollowUp.length
    ? withFollowUp.map(e => `<div class="card" onclick="openDetail('${e.id}')">
        <div class="card-row"><div class="entry-name" style="font-size:14px;">${e.procedureName}</div>${e.badge}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:6px;">Follow-up: ${formatDate(e.followUpDate)}${e.followUpNotes ? ` · ${e.followUpNotes}` : ''}</div>
      </div>`).join('')
...
function populateSettingsProfile() {
  const p = userProfile;
  const fn = document.getElementById('prof_fullName');
  if (fn) {
    fn.value = p.fullName || '';
    document.getElementById('prof_roleTitle').value = p.roleTitle || '';
    document.getElementById('prof_regNumber').value = p.regNumber || '';
    document.getElementById('prof_hospital').value = p.hospital || '';
  }
}
...
// ============================================================
function exportCSV() {
  const fields = ['date','procedureName','procedureCode','procedureType','specialty','surgeonName','roleTitle','regNumber','competencyLevel','duration','complications','outcome','followUpDate','followUpNotes','supervisorName'];
  const header = fields.map(f => `"${f}"`).join(',');
  const rows = entries.map(e => fields.map(f => `"${String(e[f]||'').replace(/"/g,'""')}"`).join(','));
...
function shareByEmail() {
  ...
  let body = `INCISIO PROCEDURE LOG%0D%0AGenerated: ${new Date().toLocaleString('en-GB')}%0D%0A`;
  if (name) body += `User: ${name}${userProfile.roleTitle ? ' · ' + userProfile.roleTitle : ''}%0D%0A`;
  body += `Total entries: ${entries.length}%0D%0A%0D%0A`;

  const list = todayEntries.length > 0 ? todayEntries : entries.slice(0,5);
  const label = todayEntries.length > 0 ? `TODAY'S ENTRIES (${formatDate(today)})` : 'RECENT ENTRIES (last 5)';
  body += `${label}%0D%0A`;
  list.forEach((e, i) => {
    body += `%0D%0A${i+1}. ${e.procedureName}`;
    if (e.specialty) body += ` (${e.specialty})`;
    body += `%0D%0A   ${formatDate(e.date)}`;
    if (e.competencyLevel) body += ` · ${e.competencyLevel}`;
    if (e.surgeonName) body += ` · With ${e.surgeonName}`;
    body += `%0D%0A`;
  });

  const subject = encodeURIComponent(`Incisio Logbook — ${formatDate(today)}${name ? ' — ' + name : ''}`);
...
window.addEventListener('offline', () => {
  ...
  toast('Offline — entries saved locally');
});
...
(async function init() {
  loadLocal();
  // Auth state is handled by onAuthStateChange listener above
  // Show auth screen while we wait
  const { data: { session } } = await sb.auth.getSession();
  if (!session) showAuthUI();
})();
