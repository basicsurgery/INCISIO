...
No explanation, no markdown, just JSON.`,
    ...
    // Surface API errors clearly instead of silently falling through
    if (!response.ok || data.type === 'error') {
      const msg = data?.error?.message || 'API error';
      console.error('Claude API error:', msg, data);
      toast('Smart Entry error — ' + msg.slice(0, 60));
    ...
    // If parsed is empty, the AI returned nothing useful
    if (!parsed || Object.keys(parsed).length === 0) {
      toast('Smart Entry returned no fields — try rephrasing');
      btn.disabled = false; btn.textContent = '✨'; return;
    }
