...
// ============================================================
// INIT — Safari-safe session recovery
// ============================================================
(async function init() {
  loadLocal();
  ...
  if (!session) {
    // Attempt token refresh before giving up (Safari PWA closes in-memory state)
    try {
      const { data: { session: refreshed } } = await sb.auth.refreshSession();
      if (!refreshed) { hideSpinner(); showAuthUI(); }
      // onAuthStateChange fires if refresh succeeds
    } catch {
      hideSpinner(); showAuthUI();
    }
  }
