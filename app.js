...
(async function init() {
  ...
  // appStarted prevents double-execution if both getSession()
  // and onAuthStateChange resolve with a session (which can happen).
  ...
  // Listener handles sign-in, sign-out, and token refresh during the session.
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_IN') {
      await startApp(session?.user || null);
  ...
  // getSession() is the reliable way to resolve the initial state on every
  // page load and refresh — reads directly from localStorage, always works.
  try {
