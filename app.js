...
// ============================================================
// SMART ENTRY — local pattern parser (no API, no cost, works offline)
// ============================================================
function parseSmartEntry(txt) {
  const t   = txt.toLowerCase();
  const out = {};

  // ── Procedure name — match against known codes first ─────
  let matchedProc = null;
  for (const p of PROC_CODES) {
    ...
    const shorthands = {
      ...
      'lap inguinal': 'Laparoscopic Hernia Repair',
      'lap hernia': 'Laparoscopic Hernia Repair',
      'open inguinal': 'Inguinal Hernia Repair',
      'lap appendic': 'Laparoscopic Appendicectomy',
      'appendic':  'Appendicectomy',
      'appy':      'Appendicectomy',
      ...
      'lap prostate': 'Radical Prostatectomy',
    ...
    for (const [short, full] of Object.entries(shorthands)) {
      if (t.includes(short) && full === p.name) { matchedProc = p; break; }
    }
    if (matchedProc) break;
    if (t.includes(p.name.toLowerCase())) { matchedProc = p; break; }
  }
  if (matchedProc) {
    out.procedureName = matchedProc.name;
    out.procedureCode = `OPCS: ${matchedProc.opcs} / CPT: ${matchedProc.cpt}`;
    out.procedureType = matchedProc.type;
    out.specialty     = matchedProc.specialty;
  }

  // ── Surgeon name — "with Mr/Mrs/Ms/Dr/Prof Smith" ────────
  const surMatch = txt.match(/with\s+(?:Mr\.?|Mrs\.?|Ms\.?|Miss\.?|Dr\.?|Prof\.?)\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/);
  if (surMatch) out.surgeonName = surMatch[0].replace(/^with\s+/i, '').trim();

  // ── Competency level ──────────────────────────────────────
  if      (/independent/.test(t))               out.competencyLevel = 'Performed (Independent)';
  else if (/supervised?/.test(t))               out.competencyLevel = 'Performed (Supervised)';
  else if (/assisted?/.test(t))                 out.competencyLevel = 'Assisted';
  else if (/observed?/.test(t))                 out.competencyLevel = 'Observed';
  else if (/scrubbed?/.test(t))                 out.competencyLevel = 'Scrubbed';

  // ── Duration — "45 min", "1.5 hours", "90 minutes" ───────
  const durMatch = txt.match(/(\d+(?:\.\d+)?)\s*(?:hr|hour|h)/i);
  const minMatch = txt.match(/(\d+)\s*(?:min|minute)/i);
  if (durMatch)      out.duration = String(Math.round(parseFloat(durMatch[1]) * 60));
  else if (minMatch) out.duration = minMatch[1];

  // ── Date ─────────────────────────────────────────────────
  if (/today/.test(t))     out.date = todayISO();
  ...
  // ── Complications ─────────────────────────────────────────
  if (/no comp|uncomplicated|nil|none/.test(t)) out.complications = 'None';
  else {
    const compMatch = txt.match(/complication[s]?\s*[:\-]?\s*([^,.]+)/i);
    if (compMatch) out.complications = compMatch[1].trim();
  }
...
function runNLP() {
  ...
  const parsed = parseSmartEntry(txt);

  if (!parsed || Object.keys(parsed).length === 0) {
    toast('Nothing recognised — try: "lap chole with Mr Smith, assisted, 45 min"');
    return;
  }
    ...
    document.querySelectorAll('.comp-chip').forEach(c => {
      ...
      if (c.textContent.trim() === parsed.competencyLevel) {
        const key = parsed.competencyLevel === 'Performed (Supervised)'  ? 'supervised'  :
                    ...
                    parsed.competencyLevel === 'Scrubbed' ? 'scrubbed' :
                    parsed.competencyLevel === 'Other'    ? 'other'    :
  ...
  toast(`${filled} field${filled !== 1 ? 's' : ''} filled ✓`);
}
...
function renderForm(data) {
  ...
  document.getElementById('formContent').innerHTML = `
    <div class="form-section">
      ...
      <div class="nlp-disclaimer">Try: "lap chole with Mr Smith, assisted, 45 min"</div>
    </div>
    <div class="form-section">
